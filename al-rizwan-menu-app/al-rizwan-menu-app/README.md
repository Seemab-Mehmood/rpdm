...
- `npm run add-settings-table` — adds the `settings` table needed for opening hours, the tagline, and the announcement banner/pop-up (see 4.10 below). Run this once if your database was seeded before that feature existed — it's safe to run again.
...
## 4.10 Opening hours, tagline & the announcement banner/pop-up

A new **⚙️ Site settings** card at the top of the admin panel controls three things, all saved instantly to the database (no redeploy needed):

- **Description / tagline** — the line shown under the restaurant name on the customer menu.
- **Opening hours** — set each day of the week to "Open 24 hours", "Custom hours" (with an open/close time — an overnight shift like 12:00 PM → 2:00 AM is fine and handled correctly), or "Closed". The customer page reads this and shows a live **OPEN NOW / CLOSED** badge with the actual reopening or closing time (e.g. "CLOSED · OPENS TOMORROW 9:00 AM"), computed in the restaurant's own timezone regardless of the visitor's device. If every day is set to 24 hours, it just shows "OPEN 24 HOURS" as before.
- **Announcement (discount banner & pop-up)** — one toggle turns both of these on/off together:
  - A **banner** pinned under the top bar on every screen of the menu, for a short message like "🎉 20% off all shawarma this weekend only!". A visitor can dismiss it with the ✕; it reappears next visit (or immediately, if you change the message).
  - A **pop-up** shown once per visit, a moment after the page loads, with a title, a longer message, and an optional button (e.g. "Order on WhatsApp" linking to your WhatsApp chat). It won't show again to the same visitor in that browser session unless you change the title/message.

If you ever see "the database may need the add-settings-table migration run" in the admin panel, run `npm run add-settings-table` once (see above).
...
scripts/add-settings-table.js — migration: adds the settings table (hours/tagline/announcement)
public/index.html    — customer-facing menu (fetches live data, no admin code at all)
public/js/site-hours.js — open/closed status logic, shared by the customer page and its tests
public/logo.jpg      — your logo, served as a normal static file
views/admin.html     — admin dashboard (only reachable via ADMIN_PATH)
tests/site-hours.test.js — `node --test tests/site-hours.test.js` to verify the hours logic
```
